// Minimal inline SVG icon set (lucide-style). Returns an SVG string.
const paths = {
...
export function icon(name, cls = "", size = 24) {
  const d = paths[name];
  if (!d) return "";
  return `<svg class="${cls}" xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${d}</svg>`;
}
